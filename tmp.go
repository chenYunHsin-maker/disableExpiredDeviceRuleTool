...
func putRequest(url string, data io.Reader) {
	...
	fmt.Println(cntUpdate, "putTo:", url, " data:", data)
...
func putToApiserver(targetUrl string, siteId string) {
	...
	case true:
		...
		//part := "\"policyRules\":["
		fmt.Println("update bname: ", siteIdToPolicyBName[siteId])
		for rows.Next() {
			var ruleName string
			var orderId string
			var source string
			...
			//part += "{\"ruleName:\":\"" + ruleName + "\"," + "\"orderId\":" + orderId + "},"
		...
		if part != "\"policyRules\":]" {
			fmt.Println("aLotOfThing: ", part)
			//fmt.Println("update bname: ", siteIdToPolicyBName[siteId])
			str = strings.Replace(str, "\"spec\":{}", "\"spec\":{"+part+"}", 1)
			if oldSpec.String() == "{}" {
				//fmt.Println("PUTDATA: ", str)
...
func main() {
	flag.Parse()
	defer glog.Flush()
	//file, err := os.Create("./bnameList" + ".txt")
	//initDb()
	/*
